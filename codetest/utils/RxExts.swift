...
extension Observable {
    ...
    func subscribeData<T: Any, R: Any>(errorObs:BehaviorRelay<String?>, mapper: @escaping((T) -> R)) -> PublishSubject<R>{
        ...
        _ = subscribeOn(ConcurrentDispatchQueueScheduler.init(queue: DispatchQueue.global())).subscribe(onNext: { data in
            
            obs.onNext(mapper(data as! T))
            
        },onError: { error in
            
            errorObs.accept((error as NSError).domain)
...
extension BehaviorRelay {
    ...
    func observe<T:Any,R:Any>(mapper: @escaping((T) -> R)) -> BehaviorRelay<R?> {
          ...
          _ = observeOn(MainScheduler.instance)
